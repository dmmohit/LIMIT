...
#pragma omp parallel for num_threads( 4 ) collapse( 2 ) private( ii, jj, kk )
	...
	for( kk = 0; kk < N_cell_x; ++kk ) Mapp[ Ind(ii,jj,kk) ] *= fac*scaling_unit ;

} // End of Intensity()

void Brightness_Temp( std::valarray<float>& Mapp ) {
	...
	const float fac = lum_den_fac * pow(Lambda, 3.0) * powf(1.0+z, 2.0) /
